...
def search_arun_knowledge(query: str) -> str:
    ...
    # 1. High Priority: Add matched project READMEs
    for folder in matched_folders:
        ...
        if os.path.exists(target_readme):
            with open(target_readme, "r", encoding="utf-8") as f:
                ...
                if content:
                    results.append(f"--- Project README ({folder}) ---\n{content}")
    ...
    # 2. High Priority: Granular LinkedIn Post Chunk Matching
    posts_path = os.path.join(data_dir, "linkedin", "posts.md")
    if os.path.exists(posts_path):
        with open(posts_path, "r", encoding="utf-8") as f:
            posts_text = f.read()
        blocks = posts_text.split("## ")
        scored_blocks = []
        for b in blocks:
            if not b.strip():
                continue
            b_lower = b.lower()
            score = sum(1 for w in significant_words if w in b_lower)
            if score > 0:
                scored_blocks.append((score, b))
        scored_blocks.sort(key=lambda x: x[0], reverse=True)
        for score, b in scored_blocks[:3]:
            results.append(f"--- Relevant LinkedIn Post Insight ---\n## {b.strip()}")

    # 3. High Priority: Granular Section Matching for Core Profile & Background Documents
    other_static_files = [
            ...
            with open(s_file, "r", encoding="utf-8") as f:
                ...
                for sec in sections:
                    if not sec.strip():
                        continue
                    sec_lower = sec.lower()
                    score = sum(1 for w in significant_words if w in sec_lower)
                    if score >= min(2, len(significant_words)):
                        scored_sections.append((score, sec.strip()))
                scored_sections.sort(key=lambda x: x[0], reverse=True)
                for score, sec in scored_sections[:2]:
                    results.append(f"--- Document Insight ({os.path.basename(s_file)}) ---\n{sec}")

    # 4. Check verified Q&A pairs in unknown_questions.json
...
def load_tutor_config(tutor_id: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    candidate_paths = [
        os.path.join(root_dir, "demos", f"{slug}_enterprise_dictionary.json"),
        os.path.join(root_dir, "demos", f"{slug}.json"),
        os.path.join(root_dir, "data", "leads", f"{slug}_enterprise_dictionary.json"),
        os.path.join(root_dir, "data", "leads", f"{slug}.json"),
        os.path.join(root_dir, "demos", "general.json"),
    ]
