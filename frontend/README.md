...
This folder contains the **Next.js 16 (Turbopack) web application** for Arun's portfolio website and AI Assistant interface. It supports both server-rendered dev mode and static HTML export (`output: "export"`) for Hugging Face Docker container hosting.
...
│   └── page.tsx            # Main state container managing active tab, streaming chat, forced Light Mode, and modals
├── components/
│   ├── Header.tsx          # Top navigation bar, tab switcher (Assistant, Projects, About), social links, theme toggle
│   ├── ChatPanel.tsx       # Landing view with 2x2 prompt cards (including live GitHub fetch card), STT Mic button, & HD TTS voice button
...
├── next.config.ts          # Static export configuration (`output: "export"`, `images.unoptimized: true`)
...
- **What it does**: Controls top-level application state. Handles NDJSON real-time token streaming from the FastAPI backend (`/chat`), forces Light Mode default on load, manages message history state, and switches between views.
...
- **What it does**: The core interactive view. Contains:
  - **Symmetrical 100% Landing Card**: Hero photo card + 2x2 question prompt grid (including live GitHub activity fetch card).
  - **Speech-to-Text (STT)**: Microphone button (`[ 🎙️ ]`) for voice-to-text input with non-blocking browser network handling.
